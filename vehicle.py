...
def process_video(input_path, output_path):
    logs = []  # Store log output for frontend rendering
            ...
            if cls in [2, 3, 5, 7]:  # vehicles
                ...
                if cy > line_y2 and obj_id in vehicle_entry_frame:
                    ...
                    if frame_count - entry_frame > 5:
                        ...
                        msg = f"Vehicle ID: {obj_id} | Speed: {speed:.2f}"
                        print(msg)
                        logs.append(msg)

                        if speed > speed_limit:
                            alert_msg = f"🚨 OVERSPEEDING → ID: {obj_id}, Speed: {speed:.2f}"
                            print(alert_msg)
                            logs.append(alert_msg)
